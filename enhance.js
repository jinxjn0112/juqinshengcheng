(() => {
    ...
    return `
      <section id="pw_enhance_box" class="pw-enhance-box">
        ...
        <div class="pw-enhance-grid">
          <label class="pw-enhance-field"><span>剧情视角</span><select id="pwe_lens">
  ...
  function bind() {
    const pairs = [
      ['#pwe_lens','lens'],['#pwe_range_mode','rangeMode'],['#pwe_latest','latestCount'],['#pwe_anchor','anchorFloor'],['#pwe_above','above'],['#pwe_below','below'],['#pwe_worldbook_mode','worldbookMode'],['#pwe_engine','engine'],['#pwe_focus','focusPoint'],['#pwe_focus_dest','focusDestination'],['#pwe_api_base','apiBase'],['#pwe_api_key','apiKey'],['#pwe_api_model','apiModel'],['#pwe_temp','temperature']
    ];
    for (const [id,key] of pairs) val(id)?.addEventListener('input', e => { cfg[key] = ['latestCount','anchorFloor','above','below','temperature'].includes(key) ? n(e.target.value, DEFAULTS[key]) : e.target.value; save(); syncVisibility(); preview(); });
    for (const [id,key] of [['#pwe_chat','readChat'],['#pwe_character','readCharacter'],['#pwe_worldbook','readWorldbook']]) val(id)?.addEventListener('change', e => { cfg[key]=e.target.checked; save(); preview(); });
  }
  function sync() {
    const values={ '#pwe_lens':cfg.lens,'#pwe_range_mode':cfg.rangeMode,'#pwe_latest':cfg.latestCount,'#pwe_anchor':cfg.anchorFloor,'#pwe_above':cfg.above,'#pwe_below':cfg.below,'#pwe_worldbook_mode':cfg.worldbookMode,'#pwe_engine':cfg.engine,'#pwe_focus':cfg.focusPoint,'#pwe_focus_dest':cfg.focusDestination,'#pwe_api_base':cfg.apiBase,'#pwe_api_key':cfg.apiKey,'#pwe_api_model':cfg.apiModel,'#pwe_temp':cfg.temperature };
  ...
  function getCharacter(context) {
    ...
    const c = chars?.[id] || context?.character || null;
    return characterText(c).slice(0,18000);
  }
  ...
  function preview() {
    ...
    if(val('#pwe_preview')) val('#pwe_preview').textContent=`本次预计读取：${floors}；人物设定 ${ch?'已找到':'未读取/未找到'}；世界书 ${w?'已找到可见信息':'未读取/当前接口未暴露'}；生成视角：${LENS_TEXT[cfg.lens]||LENS_TEXT.free}`;
  }

  function modeInstruction() {
    const active=document.querySelector('.pw-mode.is-active')?.dataset.mode || 'directions';
    const count=n(document.querySelector('#pw_count')?.value,5);
    const map={directions:`给出 ${count} 条真正不同的后续剧情走向，每条写清触发点、推动者、冲突、后果与下一幕钩子。`,new_scene:`设计 ${count} 个能直接接上当前最后一幕的新剧情方案，并各给一小段开场。`,side_story:`设计 ${count} 个与当前人物或设定有联系、但能独立成立的番外/间章。`,outline:`把后续整理为 ${count} 个章节或阶段的大纲，明确主矛盾、节拍、人物变化与章末钩子。`,foreshadow:`整理并扩展 ${count} 个伏笔机会，区分已存在的线索与新伏笔候选。`,wildcard:`生成 ${count} 个互不重复的剧情灵感，核心矛盾和推动者必须有差异。`};
  ...
  function buildPrompt(context) {
    ...
    return `你是互动剧情的剧情策划工具。基于下列实际资料生成，不要把缺失信息脑补成既定事实。\n\n【任务】\n${modeInstruction()}\n\n【剧情聚焦】\n${lens}${focus}\n\n【人物设定】\n${character||'本次未读取或未找到'}\n\n【世界书/世界信息】\n${world||'本次未读取，或当前酒馆接口没有向扩展暴露可直接读取的世界书文本。若你通过酒馆当前模型生成，可利用酒馆自动注入的世界书。'}\n\n【聊天记录】\n${chatText||'本次未读取聊天记录'}\n\n【额外要求】\n${custom||'无'}\n\n要求：优先承接最后一幕；角色行为要有动机；不要复述已经发生的剧情；主线方案要真正推进核心矛盾，支线方案要有未来可回收价值；如果抓住某个细节，要说明它如何从小点长成事件链。只输出成品，不展示思考过程。`;
  }
  ...
  function showResult(text) {
    ...
    const badge=val('#pw_result_badge'); if(badge) badge.textContent=cfg.lens==='main'?'主线推进':cfg.lens==='side'?'支线开枝':cfg.lens==='point'?'聚焦生成':'生成结果';
  ...
  function setBusy(on,msg) {
    const b=val('#pw_generate'); if(b){b.disabled=on; const s=b.querySelector('span'); if(s)s.textContent=on?'正在整理资料并生成…':'结合当前剧情生成';}
    const st=val('#pw_status'); if(st)st.textContent=msg||'';
  }
  async function generate() {
    const c=ctx(); if(!c) return toast('error','暂时无法读取酒馆上下文');
    setBusy(true,'正在按你选择的楼层、人物与世界信息生成…');
    try{ const prompt=buildPrompt(c); const out=cfg.engine==='independent'?await callIndependent(prompt):await callCurrent(prompt); showResult(out); setBusy(false,'生成完成。你可以复制、填入输入框，或调整聚焦方式再生成。'); }
    catch(e){ console.error(e); setBusy(false,`生成失败：${e.message}`); toast('error',e.message||'生成失败'); }
  }
  ...
  async function start() {
    ...
    const mo=new MutationObserver(()=>{ if(document.querySelector('.pw-options')&&!document.querySelector('#pw_enhance_box')) inject(); else replaceGenerate(); });
    mo.observe(document.body,{childList:true,subtree:true});
    console.log('[剧情灵感工坊] 增强版 v1.1.0 已加载');
  }
